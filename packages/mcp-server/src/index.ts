...
tool("asset_upload", "Upload one bounded local PNG, JPEG, or WebP as base64 and receive its content-addressed asset id.", { mimeType: z.enum(["image/png", "image/jpeg", "image/webp"]), dataBase64: z.string().max(Math.ceil((4 * 1024 * 1024) / 3) * 4) }, async (args) => client.request("/api/v1/assets", "POST", args));
