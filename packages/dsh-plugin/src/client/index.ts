...
function installRegionalBackdrops(theme: ThemePayload, state: SkinState, parentOrigin: string | null, clientInstanceId: string): RegionalBackdrop {
  ...
  const installed = regions.linked
    ? [installLinkedRegionLayer(targets, regions.main, regions.divider === true)]
    : [
      ...
      installSplitBoundary(targets, regions.sidebar, regions.main, regions.divider === true)
    ];
  ...
  return {
    ...
    dispose: () => { bridge.dispose(); conversationMask.dispose(); installed.forEach(({ layer, restore, decorations }) => { decorations?.forEach(({ dispose }) => dispose()); layer.remove(); restore(); }); restoreSidebarStacking(); }
  };
}
...
function installLinkedRegionLayer(targets: RegionTargets, backdrop: Backdrop, divider: boolean): RegionLayer {
  ...
  // The persistent stylesheet owns the divider (a sidebar ::after rule), so a
  // route transition cannot detach it. Only paint the DOM decoration when the
  // index seam is unavailable.
  const decoration = divider && !persistentBaseActive() ? createBoundaryDecoration(targets, "divider", dividerBackground()) : undefined;
  let currentTargets = targets;
  return {
    layer,
    targets: affected,
 ...
 */
function installSplitBoundary(targets: RegionTargets, sidebar: Backdrop, main: Backdrop, divider: boolean): RegionLayer {
  ...
  targets.sidebar.style.setProperty("border-right-color", "transparent", "important");
  // The persistent stylesheet owns the divider; the DOM decoration is kept
  // only as a fallback (or for the seamless blend, which CSS cannot express).
  const decoration = (divider && persistentBaseActive()) ? undefined : createBoundaryDecoration(targets, divider ? "divider" : "blend", divider ? dividerBackground() : splitBlendBackground(sidebar, main));
  if (!decoration) {
