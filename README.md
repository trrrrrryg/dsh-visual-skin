...
将下面整段复制到 **PowerShell**。它会从本仓库下载一次临时源码包、运行正式安装器，并在结束后自动删除临时文件；不会上传本机 DSH 数据。

```powershell
$bootstrap = Join-Path $env:TEMP "dsh-skin-install.ps1"
Invoke-WebRequest https://raw.githubusercontent.com/trrrrrryg/dsh-visual-skin/main/scripts/install-from-github.ps1 -OutFile $bootstrap
...
项目已提交到 DSH Hub 收录队列。Hub 中的常规“插件安装”只会安装 DSH 的渲染插件，**不会**同时部署本项目所需的 Studio Controller、Skill、MCP 与安全确认流程；因此完整安装请使用上方一键安装器。收录完成后，Hub 页面可作为发现和跳转入口。
...
# 一键安装器同样支持这些参数
powershell -NoProfile -ExecutionPolicy Bypass -File $bootstrap -DshHome C:\Users\me\.dsh -ProfileName web -ControllerPort 12000 -RestartDsh
```
</details>
